# Get API key from here: https://www.ip2location.io/
$API_KEY      = "CHANGE_ME"
    ...
    foreach ($event in $events) {
        ...
        if ($event.properties[19].Value.Length -ge 5) {
            ...
            if (-Not ($log_contents -match "$($timestamp)") -or ($log_contents.Length -eq 0)) {
                ...
                $API_ENDPOINT = "https://api.ip2location.io/?key=$($API_KEY)&ip=$($sourceIp)&format=json"
                ...
                if ($region_name -eq "") { $region_name = "null" }
                $country = $responseData.country_name
                if ($country -eq "") {$country -eq "null"}

                # Write all gathered data to the custom log file. It will look something like this:
                #
                "latitude:$($latitude),longitude:$($longitude),destinationhost:$($destinationHost),username:$($username),sourcehost:$($sourceIp),state:$($region_name), country:$($country),label:$($country) - $($sourceIp),timestamp:$($timestamp)" | Out-File $LOGFILE_PATH -Append -Encoding utf8

                Write-Host -BackgroundColor Black -ForegroundColor Magenta "latitude:$($latitude),longitude:$($longitude),destinationhost:$($destinationHost),username:$($username),sourcehost:$($sourceIp),state:$($region_name),label:$($country) - $($sourceIp),timestamp:$($timestamp)"
            }
            else {
